...
import dubaiImage1600 from './assets/optimized/hero-dubai-1600.webp'
import dubaiImage960 from './assets/optimized/hero-dubai-960.webp'
...
const services = [
  ...
  ['03', 'Creative design', 'Campaign-ready design systems that make every message unmistakably yours.'],
  ['04', 'Media production', 'Photography and video that give your product presence and momentum.'],
  ['05', 'Digital marketing', 'Performance-led campaigns, SEO and reporting that make growth visible.'],
  ['06', 'Web & app development', 'Useful, considered digital experiences built around real customer journeys.'],
]
const processSteps = [
...
const heroServices = [
  ...
  { short: 'Design', title: 'Creative design', body: 'Campaign-ready design systems that make every message unmistakably yours.', x: 91, y: 52, delay: .8, headline: ['Make every message', 'unmistakably yours.'] },
  { short: 'Media', title: 'Media production', body: 'Photography and video that give your product presence and momentum.', x: 73, y: 84, delay: 1.2, headline: ['Give ideas', 'a physical presence.'] },
  { short: 'Growth', title: 'Digital marketing', body: 'Performance-led campaigns, SEO and reporting that make growth visible.', x: 27, y: 85, delay: 1.6, headline: ['Make growth', 'visible.'] },
  { short: 'Web', title: 'Web & app development', body: 'Useful, considered digital experiences built around real customer journeys.', x: 8, y: 55, delay: 2, headline: ['Make digital', 'feel effortless.'] },
]
const heroParticles = [
...
const serviceCardMeta = [
  ...
  { icon: PenTool, values: ['Campaign-ready systems', 'Clear, cohesive messages'] },
...
const workFilters = ['All work', 'Branding', 'Creative design', 'Digital marketing', 'Media production', 'Web & apps']
const workProjects = [
...
function HeroSection() {
  ...
  const message = activeService === -1 ? { key: 'overview', kicker: 'Dubai / UAE • Full-service digital agency', headline: ['WE CREATE', 'ECHOES THAT GROW BRANDS.'], body: 'A full-service digital agency creating brands, experiences and projects that resonate and deliver measurable results.' } : { key: heroServices[activeService].title, kicker: `Service / 0${activeService + 1} • EchoVision`, headline: heroServices[activeService].headline, body: heroServices[activeService].body }
  ...
  return <section id="top" className="hero hero-tech" data-scene onPointerMove={handlePointerMove} onPointerLeave={resetPointer}>
    ...
    <div className="hero-copy">
      <motion.div className="hero-message" key={message.key} initial={reduce ? false : { opacity: 0, y: 16, filter: 'blur(5px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} transition={{ duration: activeService === -1 ? .72 : .42, ease }}>
          ...
          <h1 className="hero-title">{activeService === -1 ? <><motion.span initial={reduce ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .72, delay: .15, ease }}>WE CREATE</motion.span><motion.em initial={reduce ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .72, delay: .25, ease }}>ECHOES</motion.em><motion.span initial={reduce ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .72, delay: .33, ease }}>THAT GROW</motion.span><motion.span initial={reduce ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .72, delay: .41, ease }}>BRANDS.</motion.span></> : <><motion.span initial={reduce ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .72, delay: .05, ease }}>{message.headline[0]}</motion.span><motion.em initial={reduce ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .72, delay: .13, ease }}>{message.headline[1]}</motion.em></>}</h1>
      ...
      return <motion.article key={title} className={`service-card service-card-${index + 1} ${isFlipped ? 'is-flipped' : ''}`} role="button" tabIndex={0} aria-label={`${title}. ${isFlipped ? 'Hide details' : 'Show details'}`} aria-pressed={isFlipped} onClick={event => { if (!event.target.closest('a')) toggleCard(index) }} onKeyDown={event => onCardKeyDown(event, index)} initial={reduce ? false : { opacity: 0, y: 32, scale: .97 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: .18 }} transition={{ duration: .7, delay: index * .11, ease }}>
        <div className="service-card-inner">
          <div className="service-card-face service-card-front">
            ...
            <div className="service-card-copy"><p className="service-card-label">EchoVision capability</p><h3>{title}</h3><p>{body}</p></div><small><span>Explore</span><ArrowUpRight size={15} /></small>
          </div>
  ...
  return (
    <div className="site-shell">
      ...
      <header className="topbar">
        ...
        <nav className="desktop-nav" aria-label="Primary navigation"><a href="#top">Home</a><a href="#services">Services</a><a href="#work">Work</a><a href="#about">About us</a><a href="#contact">Contact</a></nav>
      ...
      <footer className="footer"><div className="footer-pitch"><p className="eyebrow">DUBAI / UAE — READY WHEN YOU ARE</p><h2>Have a vision?<br /><em>Let’s make it real.</em></h2><a className="footer-project" href="#contact">Start a project <ArrowUpRight size={20} /></a></div><div className="footer-end"><a className="brand" href="#top"><span className="brand-signal" aria-hidden="true" /><strong>Echo</strong>Vision<sup>®</sup></a><p>Your creative business partner in Dubai and the UAE.</p><div className="socials"><a href="https://www.instagram.com/echovision.ae/" aria-label="Instagram"><CircleDotDashed size={18} /></a><a href="https://www.facebook.com/echovision.ae" aria-label="Facebook"><Share2 size={18} /></a><a href="https://www.linkedin.com/company/echovisiondijitalmarketing" aria-label="LinkedIn"><Globe2 size={18} /></a></div><small>© 2026 EchoVision Digital Marketing. All rights reserved.</small></div></footer>
    </div>
  )
}
